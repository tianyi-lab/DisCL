...
def main(args):
    ...
    df_final['filename'] = df_final['filename'].map(lambda x: f'{args.data_dir}/{x}')
    df_final = df_final.rename({'filename': 'filepath', 'y': 'label'}, axis='columns')[['title', 'filepath', 'label']]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_file', default='../data/iwildcam/iwildcam_v2.0/train.csv')
    parser.add_argument('--english_label_path', default='./src/datasets/iwildcam_metadata/labels.csv')
    parser.add_argument('--metadata', default='../data/iwildcam/iwildcam_v2.0/metadata.csv')
    parser.add_argument('--data_dir', default='../data/iwildcam/iwildcam_v2.0/train')
