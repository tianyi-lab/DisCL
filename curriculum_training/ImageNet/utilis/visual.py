...
def sample_from_diffusion():
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    # Unet Initialization
    model = UNet_conditional(dim=64, dim_mults=(1, 2, 4), channels=1, num_classes=10)

    # diffusion Initialization
    diffusion_model = ConditionalDiffusion1D(model, seq_length=64, timesteps=1000, objective='pred_x0')
