...
model_paths = {'none': 'networks.resnet', 'tail': 'networks.resnet_ensemble'}
...
def model_init(cfg, dataset, state_dict=None):
    ...
    if cfg['ensemble_info']['name'] == 'none':
        # pdb.set_trace()
    ...
    # ----------------Raise Error with Unknown Ensemble name--------------------#
        ...
        raise Exception('Unsupported cfg[\'ensemble_info\'][\'name\'],' + 'expect in [\'none\', \'tail\']')
...
def optimizer_init(cfg, model, state_dict=None):
    if cfg['name'] == 'SGD':
        optimizer = torch.optim.SGD([{'params': model.parameters(), 'initial_lr': cfg['lr']}], lr=cfg['lr'],
                                    momentum=cfg['momentum'], nesterov=cfg['nesterov'], weight_decay=cfg['wd'])
    else:
