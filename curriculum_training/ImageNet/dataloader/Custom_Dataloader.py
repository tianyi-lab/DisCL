...
def data_loader_custm(dataset, datapath, data_transforms, imb_factor):
    ...
    if dataset == 'CIFAR10' and imb_factor == 0.01:
        ...
        class_num_list = [500, 477, 455, 434, 415, 396, 378, 361, 344, 328, 314, 299, 286, 273, 260, 248, 237, 226, 216,
                          206, 197, 188, 179, 171, 163, 156, 149, 142, 135, 129, 123, 118, 112, 107, 102, 98, 93, 89,
                          85, 81, 77, 74, 70, 67, 64, 61, 58, 56, 53, 51, 48, 46, 44, 42, 40, 38, 36, 35, 33, 32, 30,
                          29, 27, 26, 25, 24, 23, 22, 21, 20, 19, 18, 17, 16, 15, 15, 14, 13, 13, 12, 12, 11, 11, 10,
                          10, 9, 9, 8, 8, 7, 7, 7, 6, 6, 6, 6, 5, 5, 5, 5]  # cifar100 0.01
        num_classes = len(class_num_list)
        ...
        class_num_list = [500, 488, 477, 466, 455, 445, 434, 424, 415, 405, 396, 387, 378, 369, 361, 352, 344, 336, 328,
                          ...
                          132, 129, 126, 123, 121, 118, 115, 112, 110, 107, 105, 102, 100, 98, 95, 93, 91, 89, 87, 85,
                          83, 81, 79, 77, 75, 74, 72, 70, 69, 67, 66, 64, 63, 61, 60, 58, 57, 56, 54, 53, 52, 51,
                          50]  # cifar100 0.1
        num_classes = len(class_num_list)
    else:
...
def load_data(dataset_name, datapath, data_transforms, imb_factor):
    train_dataset, val_dataset, test_dataset, dset_info = data_loader_custm(dataset_name, datapath, data_transforms,
...
def Custom_dataset(args):
    ...
    dataset = {"name": args.dataset, "class_num": class_num, "imb_factor": args.imb_factor, "path": args.datapath,
               "batch_size": 64, "sampler": None, "number_worker": 0, "pin_memory": True}
    return dataset
...
def Custom_dataset_test(args):
    ...
    if args.dataset == "CIFAR100":
        class_num = 100
    dataset = {"name": args.dataset, "class_num": class_num, "imb_factor": args.imb_factor, "batch_size": 64,
               "sampler": None, "number_worker": 0, "pin_memory": True}
    return dataset
...
def data_loader_wrapper_cust(cust_dataset):
    ...
    return load_data(cust_dataset["name"], cust_dataset["path"], data_transforms=data_transforms,
                     imb_factor=cust_dataset["imb_factor"])
